...
// by driven renders through the real `DesignPlace`, network intercepted —
// scripts/drive-config-drawer.mjs's own shape, reused: a throwaway
// client/preview.html + client/src/preview.tsx (the latter copied in from
// scripts/adr0053-preview.tsx, whose own header explains why the fixture
// documents are built with the real command functions rather than through a
// UI that has no cable-connect path yet), `window.fetch` overridden inside
// the page for the four calls `useDesignSession`/`signedFetch` make, nothing
// routed around the redaction gate.
//
// Four scenes, one throwaway harness, `?scene=` picks the document:
...
const PREVIEW_SOURCE = ROOT + '/scripts/adr0053-preview.tsx';
...
writeFileSync(PREVIEW_TSX, readFileSync(PREVIEW_SOURCE, 'utf8'));
...
try {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    await page.goto(`${BASE}/preview.html?scene=trail`);
  ...
  // -------------------------------------------------------------------------
  {
    ...
    const pageErrors = [];
    page.on('pageerror', (e) => pageErrors.push(e.message));
    await page.goto(`${BASE}/preview.html?scene=conflict`);
    await page.waitForSelector('.react-flow__edge-cable', { timeout: 15_000 });
    ...
    check(
      ...
      refusal.some((t) => t.includes('add note') && t.includes('account-maria-p')),
      refusal.join(' | '),
    );
  ...
  // -------------------------------------------------------------------------
  {
    ...
    const pageErrors = [];
    page.on('pageerror', (e) => pageErrors.push(e.message));
    await page.goto(`${BASE}/preview.html?scene=note`);
  ...
  // -------------------------------------------------------------------------
  {
    ...
    const pageErrors = [];
    page.on('pageerror', (e) => pageErrors.push(e.message));
    await page.goto(`${BASE}/preview.html?scene=typed`);
    await page.click('.react-flow__node-chassis >> nth=0');
    await page.waitForSelector('.drawing-editor__panel', { timeout: 10_000 });
