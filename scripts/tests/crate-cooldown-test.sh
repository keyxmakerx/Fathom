...
check ok "a first-party-only lockfile passes" \
      env COOLDOWN_LOCK="$TMP/first-party.lock" COOLDOWN_EXCEPTIONS="$TMP/none.md" "$COOLDOWN"
...
EOF
out=$(COOLDOWN_LOCK="$TMP/old.lock" COOLDOWN_EXCEPTIONS="$TMP/none.md" \
      "$COOLDOWN" 2>&1) && rc=0 || rc=$?
case "$out" in
...
out=$(COOLDOWN_LOCK="$TMP/old.lock" COOLDOWN_DAYS=99999 \
      COOLDOWN_EXCEPTIONS="$TMP/none.md" "$COOLDOWN" 2>&1) && rc=0 || rc=$?
if [ "$rc" -ne 0 ]; then
...
# ---- EXPIRING EXCEPTIONS ----
# A young crate, and an exception file that admits it, expires it, or has
# outlived it. Driven against a real young release rather than a fixture: the
# lockfile below pins a version that IS young, so the window really is the thing
# being overridden.
cat > "$TMP/young.lock" <<'EOF'
version = 4
...
# cfg-if 1.0.0 is years old, so a 99999-day window makes it "young" on demand
# without depending on some crate happening to have been published this week.
wide() { env COOLDOWN_LOCK="$TMP/young.lock" COOLDOWN_DAYS=99999 \
             COOLDOWN_EXCEPTIONS="$TMP/exceptions.md" "$COOLDOWN"; }

: > "$TMP/exceptions.md"
check fail "a young crate with no exception fails" wide

cat > "$TMP/exceptions.md" <<EOF
<!-- cooldown:exceptions -->
| crate | version | expires | why |
|---|---|---|---|
| \`cfg-if\` | 1.0.0 | $(date -u -d "+30 days" +%Y-%m-%d) | fixture |
<!-- cooldown:end -->
EOF
check ok "an unexpired exception admits it" wide

cat > "$TMP/exceptions.md" <<EOF
<!-- cooldown:exceptions -->
| crate | version | expires | why |
|---|---|---|---|
| \`cfg-if\` | 1.0.0 | $(date -u -d "-1 day" +%Y-%m-%d) | fixture |
<!-- cooldown:end -->
EOF
check fail "an EXPIRED exception fails rather than lapsing quietly" wide

# The version is part of the key: an exception for one version must not cover
# the next one. This is the property that stops a row silently outliving its
# reason.
cat > "$TMP/exceptions.md" <<EOF
<!-- cooldown:exceptions -->
| crate | version | expires | why |
|---|---|---|---|
| \`cfg-if\` | 0.9.0 | $(date -u -d "+30 days" +%Y-%m-%d) | fixture, wrong version |
<!-- cooldown:end -->
EOF
check fail "an exception for a different version does not cover this one" wide

# A row outside the markers is prose.
cat > "$TMP/exceptions.md" <<EOF
| \`cfg-if\` | 1.0.0 | $(date -u -d "+30 days" +%Y-%m-%d) | outside the markers |
EOF
check fail "an exception row outside the markers admits nothing" wide

# ...and once the crate is old enough on its own, the row is DEAD and the
# script says so rather than letting the file accumulate.
cat > "$TMP/exceptions.md" <<EOF
<!-- cooldown:exceptions -->
| crate | version | expires | why |
|---|---|---|---|
...
<!-- cooldown:end -->
EOF
out=$(COOLDOWN_LOCK="$TMP/young.lock" COOLDOWN_EXCEPTIONS="$TMP/exceptions.md" \
      "$COOLDOWN" 2>&1) && rc=0 || rc=$?
case "$out" in *"no longer needs its"*) said=1 ;; *) said=0 ;; esac
if [ "$rc" -ne 0 ] && [ "$said" -eq 1 ]; then
    pass=$((pass + 1)); echo "  ok    an exception that has outlived its reason fails, and says to remove it"
else
    fail=$((fail + 1)); echo "  FAIL  an exception that has outlived its reason fails (exit $rc)"
    echo "$out" | sed 's/^/        /'
fi
...
out=$(PATH="$TMP/bin:$PATH" COOLDOWN_LOCK="$TMP/old.lock" \
      COOLDOWN_EXCEPTIONS="$TMP/none.md" COOLDOWN_ALLOW_UNREACHABLE=1 \
      "$COOLDOWN" 2>&1) && rc=0 || rc=$?
if [ "$rc" -eq 0 ]; then
