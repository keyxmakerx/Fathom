...
# from the environment. Until 2026-09-19 this script also GENERATED the two
# database passwords; it runs as the `postgres` user, and the volume it
# wrote into is created root-owned, so that write was refused on a clean
# checkout. `keys-init` in `compose.yaml` now generates every secret this
# stack needs before this container starts; this script only reads.
#
...
# since 2026-09-19 `keys-init` writes them and this script reads them.
#
# WHY THE RUNTIME PASSWORD IS GENERATED TOO, EVEN THOUGH THE RUNTIME ROLE
# DOES NOT EXIST YET WHEN THIS RUNS (generation moved to `keys-init`, 2026-09-19). `fathom_app` is created by
