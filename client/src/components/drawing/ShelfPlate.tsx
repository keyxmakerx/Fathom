...
export interface ShelfPlateNodeData extends Record<string, unknown> {
  ...
  elevation: Facing;
  selected: boolean;
  /** UI-SPEC "Places · Shelf": "Empty slots draw as hatched gaps... from the
   * shelf's slot count if the catalogue gives one, else occupants only."
   * Neither `ShelfView` nor `api/catalogue.ts` carries a shelf's own slot
   * capacity yet (this session's CONTRACT does not add one) — `null` until
   * a caller can supply one, which draws occupants only, the spec's own
   * named fallback. */
  slotCount: number | null;
  /** Which occupant (if any) is open at the faceplate stop — UI-SPEC
   * "Places · Shelf" / "Motion" #10: "A box on a shelf opens at the
 ...
 * `PortRow` uses) and the faceplate-stop inset (UI-SPEC "Places · Shelf":
 * "full glyph size with labels," `counterScaledGlyphScale` — the same "true
 * size" a `ChassisNode`'s own faceplate reads at, never shrunk). Each glyph
...
/** One occupant box at the rack stop — UI-SPEC "Places · Shelf": "its
 * occupants as named boxes left to right by slot, each box showing its
 * model or 'sketch', a dotted TYPED mark on a sketch." */
function OccupantBox({
  ...
  // UI-SPEC "Places · Shelf": patch-facing gear shows its ports at the rack
  // stop; everything else shows ports only once opened (the inset below).
  ...
  return (
    ...
    >
      <span className="drawing-shelf__occupant-header">
        ...
        {occupant.sketch && (
          // UI-SPEC "Places · Shelf": "a dotted TYPED mark on a sketch" — a
          // fact about provenance, not risk, so it carries no colour
...
/** The faceplate-stop inset — UI-SPEC "Places · Shelf": "at the faceplate
 * stop an occupant opens beside the shelf showing its ports at full glyph
 * size with labels (the board's inset)." Positioned outside the plate's own
 * box (`shelf.css`'s `.drawing-shelf__inset`, `left: 100%`) so it reads as
 * something opening beside the row rather than inside it, exactly the Shelf
...
/** The shelf — UI-SPEC "Places · Shelf": "a shelf draws in the rack at its
 * units as a plate labelled with the shelf's label and 'SHELF <height>U',
 * its occupants as named boxes left to right by slot." A sibling React Flow
 * node aligned to the rack's own frame exactly as `ChassisNode.tsx` is
...
export function ShelfPlate({ data }: NodeProps<ShelfPlateNodeType>) {
  ...
  // UI-SPEC "Places · Shelf": "from the shelf's slot count if the catalogue
  // gives one, else occupants only" — see `ShelfPlateNodeData.slotCount`'s
  // own doc.
    ...
    >
      ...
      <div className="drawing-shelf__occupants">
        {slotNumbers.map((slot) => {
          ...
          if (!item) {
            // UI-SPEC "Places · Shelf": "Empty slots draw as hatched gaps
            // the way free units do" — `.drawing-rack__free`'s own hatch,
            // `shelf.css`'s `.drawing-shelf__gap` shares its token.
      ...
      {/* A bundle band's one shared anchor per shelf, the same trick
          `ChassisNode.tsx`'s own `__bundle__` handle uses, so a bundled
          cable can still anchor to "the shelf" as a whole at the closet
          stop even before a per-occupant handle is resolvable. */}
