// What the account screen says when an app-code act is refused.
//
...
/**
 * **409 is the typed refusal for "this account already has an app code"**
 ...
 * verbatim: replacing an app code is a recovery and not a form, and the
 * uniform *"sign-in refused"* reads as though the person had got something
 * wrong.
 *
...
export function describeAppCodeRefusal(error: unknown): string {
  if (error instanceof ApiRefusal && error.status === 409) {
    ...
    return 'This account already has an app code. Replacing one is a recovery, not a form: it goes through the host command.';
  }
