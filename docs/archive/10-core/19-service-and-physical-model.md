...
|---|---|---|
| **Rack** | **Added 2026-08-15 — ADR-0036** | This row's own prediction, paid: `Chassis --MountedIn--> Rack` is exactly what shipped — one kind, one reference edge, plus `HasRack` (`Premises → Rack`, containment) so the rack hangs off a place the way `PassiveNode` does. Priced a **minor** bump against `62` §16.2 and measured at **+2,794 module bytes** for the schema half on the tip -- it was byte-neutral before ADR-0035 introduced the `Placeable` class, and admitting `Rack` to that class is what the 2,794 buys. The omission test was applied correctly and held: nothing was restructured and no kind's containment owner changed. Still nothing in `77` traverses a rack — the justification is the owner's request, not a traversal need |
| **Rack unit, position, height, face** | **Added 2026-08-15 — ADR-0036** | As **fields on the `MountedIn` edge**, not on `Chassis`: "at U12, front" is a fact about the mounting, and a chassis on a shelf has no U. `position_u` is required, `height_u` and `face` are optional and an unstated height is drawn as 1U and *marked* rather than recorded as 1U. The data-entry-burden objection stands and is why nothing demands them — but the burden is the operator's to choose, and refusing the field refused the choice |
