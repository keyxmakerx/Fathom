...
| 11 | WO-11 | `WO-11-the-server-skeleton-and-the-dependency-gate.md` | **OPEN — authored 2026-09-03. The first server-side order and phase 1's first row.** Phase 0 closed the same day (ADR-0040 ratified key custody, `49` §22 decision 1). **Carries one owner act that cannot be delegated (§5 step 0) and is BLOCKED until it happens**: ADR-0032 §5 makes crate approval undelegatable, and this order is where zero external dependencies becomes ~109 crates. Builds the smallest server that proves the stack — a binary, an honest `/health`, PostgreSQL, migrations, a compose file — and spends its real effort on the gate that has to survive them. Stores NOTHING: ADR-0040's key boundary is undecided (§9 items 1-2), so a stored row here would be the retrofit that ADR forbids | — | `fathom-server`: a health-checking binary on PostgreSQL, plus `cargo audit` in CI and a gate-zero that knows the closure-document pattern — proved by watching both fail |
