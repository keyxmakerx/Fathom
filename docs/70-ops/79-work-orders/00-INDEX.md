...
| 10 | WO-10 | `WO-10-dhcp-relay-and-bootp.md` | **READY, BLOCKED ON BYTES** — nothing is undecided; the modelling, the field keys, the statement set and the gates are written. It is waiting on room in the module and the block is MEASURED: a `DhcpRelay` kind costs +602 against 219 free, and even the two-field compromise its §4 rejects on merit misses by 27. Flip to OPEN when the build reports ≥1,200 bytes of headroom | the byte decision (owner) | `DhcpRelay` as a kind, `HasDhcpRelay` and `RelaysFor` as edges, and `corpus/dict/junos-srx/forwarding-options.yaml` binding both the `helpers bootp` and the `dhcp-relay` forms — the first statements the estate cannot hold, and the first feature the owner asked for by name that the ceiling refuses |
