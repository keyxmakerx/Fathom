...
| 10 | WO-10 | `WO-10-dhcp-relay-and-bootp.md` | **OPEN as of 2026-08-28** — its blocker was the 900,000-byte ceiling, and the ceiling was removed 2026-08-21 (`49` §1). Its own flip condition ("≥1,200 bytes of headroom") is satisfied vacuously: there is no ceiling to have headroom against. Nothing is undecided — the modelling, the field keys, the statement set and the gates were written while it waited. The +602 measurement stands as history of why it waited | — | `DhcpRelay` as a kind, `HasDhcpRelay` and `RelaysFor` as edges, and `corpus/dict/junos-srx/forwarding-options.yaml` binding both the `helpers bootp` and the `dhcp-relay` forms — the first feature the owner asked for by name that the ceiling refused, now unrefused |
