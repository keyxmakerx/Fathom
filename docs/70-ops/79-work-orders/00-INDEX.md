...
| 10 | WO-10 | `WO-10-dhcp-relay-and-bootp.md` | **BLOCKED ON PLANNING as of 2026-08-28** — run the same day, stopped at Step 0 before any schema edit. Two independent, dated Juniper sources confirm `routing-instance` may qualify an individual `server` statement under `helpers bootp`, which `DhcpRelay` as spelled carries no edge for — the order's own §10 item 3 trigger, fired as written. Waits on a planning decision among three named routes (a `RoutingInstance` edge, a field, or excluding the qualified form from the first cut); nothing else in the order is undecided | — | `DhcpRelay` as a kind, `HasDhcpRelay` and `RelaysFor` as edges, and `corpus/dict/junos-srx/forwarding-options.yaml` binding both the `helpers bootp` and the `dhcp-relay` forms — the first feature the owner asked for by name that the ceiling refused, now waiting on one further modelling decision instead |
