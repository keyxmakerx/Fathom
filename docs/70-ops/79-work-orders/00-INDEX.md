...
| 11 | WO-11 | `WO-11-the-server-skeleton-and-the-dependency-gate.md` | **DONE 2026-09-03** — executed in one session, commit per step; §9 is the as-built note. **Every one of the five gate layers found something on a real arrival**: gate-zero separated direct from transitive on all four, `cargo deny` caught two duplicate pairs (skipped by exact version, never relaxed to `warn`), and the cooldown caught four young crates — three of them crates nobody chose, one of them published THAT DAY. `hyper 1.11.1` could not simply be pinned back (four HTTP/1 parser fixes where a differential is request smuggling), so the cooldown grew **expiring per-version exceptions** rather than a lowered window. Two records were CORRECTED by what the gates found: `tracing.md`'s "attributes is off" was false within one commit (feature unification via `deadpool-postgres`), and the URL redactor emitted a password when the `@` was left out. **Stores nothing**, proved twice. **One escalation stays open and is the owner's**: at 115 crates with four of `49` §6's sixteen rows in, `35` §5.1's ≤160 will not survive phase 1 — §9.7 names three routes and chooses none | — | `crates/fathom-server` (health on PostgreSQL, 41 tests), `deploy/` (compose, distroless Dockerfile, Caddy in front), the five-layer gate in CI, and two browser-free drivers: 17/17 against a real stopped-and-restarted PostgreSQL, 20/20 against the composed stack over verified TLS |
| 12 | WO-12 | `WO-12-the-key-boundary-and-the-first-stored-row.md` | **OPEN** — the second server order and **the first row of customer data this project has ever stored**, which is why it explicitly **supersedes WO-11 G8** (*"nothing is stored"*) and repurposes its test into a column census. It may proceed with `OPEN-FOR-THE-OWNER.md` §A1 open because ADR-0040 D1–D4 already decided the *architecture* of custody and all three §A1 answers are opaque bytes in one provider-neutral column; **it decides nothing in §A or §B** and carries fourteen stop-and-escalate triggers keyed to those questions. **Read its header block and Disagreements 5 first**: ADR-0040 §9 item 1 says *"for planning, before the first migration"* and this order writes that migration — the sequencing is escalated to planning in the PR body, not argued away, and if planning rules for the clause the order stops after step 4 with nothing stored | WO-11 | The wrap point: `keys/` (the `FSL1` envelope, `MasterKeyProvider`, one unwrap path, a `file` provider for dev and test), migration `0002` (five tables, 27 columns, every key on the four tenant-scoped tables leading with `tenant_id` — composite on the three that have a second column, and `tenant`'s single-column primary key *is* `tenant_id`; `master_key_probe` is the deployment's and has none — no free text), and the one test the order is shaped backwards from — **a custody switch under a second, differently-shaped provider leaving `design.wrapped_key` and `design_blob.sealed` byte-identical** |
