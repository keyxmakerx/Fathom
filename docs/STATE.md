...
shuts down cleanly, and runs as the one plain-HTTP published port of a composed stack, behind the
operator's own TLS-terminating proxy. It stores identity and structure —
...
- **A save carries its base; a drawer creates a design; a steward creates a scope** (Session 7,
  ...
  handlers verify authority once per organisation; the compose image copies the corpus and, since
  2026-09-20, the built client, which the server serves itself; the single-operator flag is read at
...
(`docs/RUNNING-IT.md`): it pulls the image `publish.yml` pushes to GHCR from every merge to
`main`, and `--build` builds the same stage from the checkout. The file is self-contained: both
first-start scripts are inline `configs`, so it can be pasted into a compose front end such as
Arcane with only the one variable beside it. A one-shot `keys-init` container generates the two
root keys and the three database passwords into the `keys` volume, which closed the last two
first-start faults: keys that had to be generated by hand into a read-only volume, and a database
init script that could not write into a root-owned one. **As of 2026-09-20 a client's address is one rule for every route** (`client_address.rs`):
`X-Forwarded-For` believed only from `FATHOM_TRUSTED_PROXIES`, across every line of the header (a
proxy that adds a line writes last); two routes had read the header's first entry, the one a
client writes. **Corrected 2026-09-21 by the review:** the rule skipped every entry inside a
trusted range, so with `private` a client on the LAN or the overlay collapsed onto the proxy's
address and a forged entry in front could stand. Now the client is the last entry the trusted
proxy appended, or the `FATHOM_FORWARDED_HOPS`-th from the right behind a further hop; nothing is
trusted by default, so out of the box the peer is the address and the server warns; and a header
named with no proxy trusted refuses to start instead of believing every peer. The built client is
read into memory once at startup rather than from disk per request. **The operator
console can be confined** to host names and source ranges
(`FATHOM_ADMIN_HOSTS`, `FATHOM_ADMIN_SOURCES`; elsewhere its paths are 404, the site unaffected).
**As of 2026-09-20 the server serves the
web client itself** (`crates/fathom-server/src/client.rs`, `FATHOM_CLIENT_ROOT`) and is the one
published port, plain HTTP, for the operator's own reverse proxy; the Caddy that served the client
and terminated TLS for one day is gone, and HTTPS is the proxy's job, which the browser's
WebCrypto needs for sign-in. `.github/workflows/ci.yml`'s `compose` job builds the image, starts
the stack, checks health and the client on the published port, reads the first-operator token
and proves a restart keeps its keys. The root `Dockerfile` and `docker-compose.yml`, which
packaged the retired single-file client, are deleted; `publish.yml` publishes
`deploy/Dockerfile`'s `server` stage as `ghcr.io/keyxmakerx/fathom-server`, and runs its test
floor against a database, which it never had.
