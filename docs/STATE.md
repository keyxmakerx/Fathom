...
repository root; `docker compose up -d --build` with one variable in `.env` is the whole
instruction (`docs/RUNNING-IT.md`). A one-shot `keys-init` container generates the two root keys
