...
`main`, and `--build` builds the same stages from the checkout. The file is self-contained: the Caddyfile and both first-start
scripts are inline `configs`, so it can be pasted into a compose front end such as Arcane with
only the one variable beside it. A one-shot `keys-init` container generates the two root keys
