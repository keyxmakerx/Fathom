...
| 5.12 | **The finder's candidate cap selects by entry ordinal, not by score** (added 2026-08-07, correcting §13 item 2a). `crates/fathom-find/src/lib.rs:131` is `candidates.into_iter().take(CANDIDATE_CAP)` over a `BTreeSet<u32>`, so the 1,024 kept candidates are the 1,024 **lowest entry ordinals** — corpus insertion order — not the best-scoring ones. Syntax hits carry `W_SYNTAX = 2.0`, the heaviest weight in the model, and are unioned into the same set, so an exact command-name match on a high-ordinal entry can be dropped in favour of 1,024 weak concept-posting entries. `16` §9.3 justifies the cap with *"beyond that the query is not selective and the extra entries cannot reach the cutoff anyway"*, which is an assertion, not a proof, and the syntax path is the counter-example. **Latent, not live:** the corpus holds 177 entries (98 + 37 + 42), so the cap cannot fire until roughly a six-fold growth — which is exactly when it stops being observable | Either cap after scoring, or apply `16` §9.3's own per-source sub-caps (concept 600, syntax 64 + 32) which this function does not implement. Belongs to WO-06 (finder completion); the spec question — whether §9.3's justification survives the syntax counter-example — is planning's. **Not to be fixed by a drive-by**: WO-06 owns this crate and `78` §5 forbids widening an order in flight |
...
| ~~**Q3**~~ | **ANSWERED 2026-08-06** — Juniper is primary, not retired. The owner's platforms are SRX, MX, EX, Nexus, Palo Alto and Meraki (`70` §7, verbatim). ADR-0029 stays live and **ADR-0030 is vindicated** — Palo Alto is on the owner's own list, so §5.9 narrows to *when*, not *whether*. One new question replaces it: `70` §11.3, on whether Meraki is configurable by pasteable text at all | §5.9 narrowed |
...
2. **Against sixteen findings this review itself raised and then discarded.** Recorded so they are
   ...
   entries by alphabetical id"* — the code shape was right and the mechanism was wrong. **This
   refutation was itself too broad, and is corrected at §5.12 (2026-08-07): the mechanism is not
   alphabetical, but the cap does select by entry ordinal rather than by score, and that half of the
   finding survives.** The discard was right to reject the stated mechanism and wrong to take the
   concern with it; (b) *"`18`'s
