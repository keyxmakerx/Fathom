...
**One final note on sequencing, which cuts across all of the above.** Every platform here multiplies devices, and `70` §6 names the largest requirement in the corpus with no mechanism behind it: **automatic correlation across separately-pasted configs.** *(As of 2026-08-21 `OP_PASTE` is additive — the estate holds many devices, and a paste naming a box it already holds asks rather than merging; the correlation itself is still unbuilt.)* A fifth platform on top of an estate whose merge question is still open is worth less than the second device. If forced to choose between platform four and correlation, choose correlation.
