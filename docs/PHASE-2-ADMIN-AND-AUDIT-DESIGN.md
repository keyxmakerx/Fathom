...
`authority_rollback`, `heartbeat`, and **`rewrap`** — on both lists, decided 2026-09-12 after a
checker showed the first build re-wrapping one tenant's keys while retiring the deployment-wide
master key, which left every other tenant unopenable. A re-wrap is deployment-wide because the master
key is: the site entry records the act (old and new key id, tenants re-wrapped) and each
organisation's entry records the fact for that tenant, as storage §12.6 requires. One transaction
covers all of it, so there is no key change without its entries and no entry without its key change.
