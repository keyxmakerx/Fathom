...
2. **While pending, the client shows the setup flow and nothing else.** No sign-in page, no links.
   ...
   5. *Sign in with your new authenticator.* One field, the verification code; the flow signs in
      with the address and password it still holds, ending the setup-only session first, so the
      session that lands on Home is one that proved the second factor (the operator key needs
